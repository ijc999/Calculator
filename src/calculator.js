    ...
    <div className='calculator'>
      <div className='c-wrapper'>
        ...
        <div className='c-grid'>
          {["ac", "pm", "", "/"].map((value, i) => (
            <button key={i} type="button" className={`top-btn${value === "" ? " special" : ""}`} value={value} onClick={handleOperator}>
              {value === "pm" ? "±" : value}
            </button>
          ))}
          ...
          ["1", "2", "3"],
            ...
            <>
              {row.map((value, j) => (
                <button key={j} className="normal" value={value} onClick={handleOperand}>
                  {value}
                </button>
              ))}
              <button className="special" value={["+", "*", "-"][i]} onClick={handleOperator}>
                {["+", "x", "-"][i]}
              </button>
            </>
          ))}
          ...
          <button
            className="special" value="=" onClick={handleOperator}>
            =
          </button>
        </div>
      </div>
